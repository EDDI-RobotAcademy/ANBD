...
public class EvPageMaker {
	...
	private int totalCount; // 게시글 총 갯수
	   ...
	   private EvCriteria cri; // 페이지 정보 객체
	   ...
	   public EvCriteria getCri() {
	      return cri;
	   }
	   public void setCri(EvCriteria cri) {
	      this.cri = cri;
	   }
	   ...
	   public String makeSearch(int page) { // list.jsp에서 페이지 숫자 눌렀을 때 실행되는 메소드
	      UriComponents uriComponents = 
	            ...
	            */
	            ...
	            .queryParam("searchType", ((EvSearchCriteria)cri).getSearchType())
	            .queryParam("state", ((EvSearchCriteria)cri).getState())
	            .queryParam("keyword", encoding(((EvSearchCriteria)cri).getKeyword()))
	            .build();
